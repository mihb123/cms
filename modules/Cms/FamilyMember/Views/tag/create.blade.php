...
@section('content')
    <form class="kt-form" id="node-form" action="{{ route('family-member-tag.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row">
            <div class="col-md-9">
                <div class="kt-portlet">
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="title"placeholder="@lang('こちらにタイトルを入力')" value="{{ old('title') }}">
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control status" data-placeholder="選択してください" name="tag_group_family_member_id">
                                ...
                                @foreach($listGroup as $key => $group)
                                    <option {{ (old('tag_group_family_member_id') == $group['id']) ? 'selected' : '' }} value="{{ $group['id'] }}">{{ $group['title'] }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
