...
@section('content')
    <form class="kt-form" id="node-form" action="{{ route('posts-topic.store') }}" method="POST">
        @csrf
        <div class="row">
            <div class="col-md-9">
                <div class="kt-portlet">
                    ...
                    <div class="kt-portlet__body">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="summary" placeholder="{{ __('posts::messages.summary') }}" cols="30" rows="3">{{ old('summary') }}</textarea>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control status" data-placeholder="{{ __('posts::messages.position') }}" name="position">
                                ...
                                @foreach(getListPositionPostTopic() as $key => $postTopic)
                                    <option {{ (old('position') == $key) ? 'selected' : '' }} value="{{ $key }}">{{ $postTopic }}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="kt-portlet">
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group">
                            ...
                            @upload(['type' => 'image', 'vendor' => 'posts-topic', 'value' => old('avatar') ?? '' , 'thumb' => old('thumb-avatar') ?? '','name' => 'avatar'])
                        </div>
                    </div>
                    <div class="kt-portlet__head">
                        <div class="kt-portlet__head-label">
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group">
                            ...
                            @upload(['type' => 'image', 'vendor' => 'posts-topic', 'value' => old('icon') ?? '' , 'thumb' => old('thumb-icon') ?? '','name' => 'icon'])
                        </div>
                    </div>
                </div>
                <div class="kt-portlet">
