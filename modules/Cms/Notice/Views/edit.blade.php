...
@section('content')
    <form class="kt-form row" id="node-form" action="{{ route('notice.update', $notice['id']) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="kt-portlet">
                ...
                <div class="kt-portlet__body">
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="title" placeholder="@lang('notice::messages.title')" value="{{ $notice['title'] }}">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control status" data-placeholder="タググループ" name="notice_category_id">
                            ...
                            @foreach($noticeCategories as $key => $noticeCategorie)
                                <option {{ ($notice['notice_category_id'] == $noticeCategorie['id']) ? 'selected' : '' }} value="{{ $noticeCategorie['id'] }}">{{ $noticeCategorie['title'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control status" data-placeholder="{{ __('notice::messages.status') }}"name="status">
                            ...
                            @foreach(getListStatus() as $value => $name)
                                <option {{ ($notice['status'] == $value) ? 'selected' : '' }} value="{{ $value }}">{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
