...
@section('content')
    <form class="kt-form" id="node-form" action="{{ route('calculate-category.update', $data['id']) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control status" data-placeholder="選択ください。" name="group_id[]" multiple="multiple">
                                @foreach($listGroup as $key => $group)
                                    <option {{ in_array($group['id'],$groupIds) ? 'selected' : '' }} value="{{ $group['id'] }}">{{ $group['title_japan'] }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group">
                            <div class="col-md-12 fvalue">
                                <a class="btn btn-dark" href="{{ route('calculate-category.index') }}">
