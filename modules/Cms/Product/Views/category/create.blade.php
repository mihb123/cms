...
@section('content')
    <form class="kt-form" id="node-form" action="{{ route('product-category.store') }}" method="POST"
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control status" data-placeholder="{{ __('product::messages.key_words') }}"
                                ...
                                @foreach ($listKeyWords as $key => $keyWord)
                                    <option
                                        {{ old('keyWord') && in_array($keyWord->id, old('keyWord')) ? 'selected' : '' }}
                                        value="{{ $keyWord->id }}">{{ $keyWord->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control status" data-placeholder="{{ __('誘導先') }}"
                                ...
                                @foreach ($listDestinationGuide as $value => $destinationGuide)
                                    <option {{ old('destination_guide_id') == $destinationGuide->id ? 'selected' : '' }}
                ...
                <div class="kt-portlet">
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group">
                            ...
                            @upload(['type' => 'image', 'vendor' => 'productCategory', 'value' => old('avatar') ?? '', 'thumb' => old('thumb-avatar') ?? '', 'name' => 'avatar'])
                        </div>
                    </div>
                </div>
                <div class="kt-portlet">
